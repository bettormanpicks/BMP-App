...
import os
import json
import re  # Added for regex safety net
...
def get_live_session(driver):
    ...
    # Safety Net: Upgraded page source regex for universal league compatibility
    try:
        source = driver.page_source
        # 💡 Chg: Case-insensitive flags (?i) and looser spacing to match any league layout variation
        token_match = re.search(r'(?i)"x-api-token"\s*:\s*"([a-z0-9]+)"', source)
        ts_match = re.search(r'(?i)"x-api-timestamp"\s*:\s*"([0-9]+)"', source)
        
        if token_match:
            token = token_match.group(1)
            timestamp = ts_match.group(1) if ts_match else str(int(time.time()))
            print(f"✅ Extracted token from page source fallback: {token}")
            return {"token": token, "timestamp": timestamp}
    except:
        pass
    return None
        ...
        data = driver.execute_script("""
            return fetch(arguments[0], {
                headers: {
                    ...
                    "x-country": "us",
                    "referer": "https://scores24.live/en/table-tennis/l-czech-liga-pro-1"
                },
                credentials: "include"
        ...
        for edge in edges:
            try:
                ...
                buffer.append({
                    ...
                    "player1": player1, "player2": player2, "match_link": match_link
                })
