...
# --- Load CSVs ---
matchlogs = pd.read_csv("data/tt_czech_matchlogs.csv", parse_dates=["match_date"])
schedule = pd.read_csv("data/tt_czech_schedule.csv", parse_dates=["match_date"])
...
if rows:
    ...
    df_out.to_csv("data/bestczechwagers.csv", index=False)
    print("Saved best wagers table to data/bestczechwagers.csv")
else:
